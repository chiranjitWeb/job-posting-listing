...
//add_action('wpcf7_before_send_mail', 'save_application_data1');

//add_action('init', 'save_application_data1');

// function save_application_data1() {
//    // require_once( wp_normalize_path(ABSPATH).'wp-load.php');
//    if ( ! defined( 'ABSPATH' ) ) {
//     require_once( wp_normalize_path(ABSPATH) . 'wp-load.php' );
// }
//     if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['cv-file'])) {
//         $cv_file = $_FILES['cv-file'];
//         //$upload_dir = 'C:/uploads/';  // Change the path to the new directory
//         //$upload_path = $upload_dir . basename($cv_file['name']);
//         $upload_dir = wp_upload_dir();
//         $custom_dir = $upload_dir['basedir'] . '/applications'; // e.g., wp-content/uploads/applications
...
//             // Get the file name and move the uploaded file to the custom directory
//             //$file_name = rand(100).basename($cv_file['name']);
...
//             }
//             $applicant_data = array(
//                             'name' => sanitize_text_field($_POST['text-571']),
//                             'email' => sanitize_email($_POST['email-416']),
//                             'dob' => sanitize_text_field($_POST['datepicker-815']),
//                         );
                
//                         $post_data = array(
//                             'post_title' => 'Application from ' . $applicant_data['name'],
//                             'post_status' => 'publish',
//                             'post_type' => 'job_application',
//                             'meta_input' => array(
//                                 'job_id' => $_POST['job-id'],
...
//                                 'applicant_dob' => $applicant_data['dob'],
//                                 'applicant_dob' => $applicant_data['dob'],
//                                 'cv_file'           => esc_url($upload_dir['baseurl'] . '/applications/' . $file_name), // Store the file URL
...
//         } else {
//             echo "File upload error: " . $cv_file['error'];
//         }
//     } 
// }

// if ($_SERVER['REQUEST_METHOD'] === 'POST') {
//   save_application_data1();
// }
...
function process_application_data($contact_form) {
    ...
    if ($submission) {
        ...
        //   $uploaded_file_url = upload_cv_file($v);
        //   if($uploaded_file_url){
        //     echo $uploaded_file_url;die();
        //   }else{
        //     echo 'not f';die();
        //   }
       ...
       insert_application_post($posted_data, $uploaded_file_url);
        ...
        // if ($uploaded_file_url) {
        //     // Call the function to insert the application post
        //     //insert_application_post($posted_data, $uploaded_file_url);
        // } else {
 ...
 */
function upload_cv_file($v) {
    // Get uploaded files from the form
    //$cv_file = $_FILES[$file_input_name];
    $cv_file = $_FILES['cv-file'];
   // print_r($cv_file);die();

    // Ensure there is a file and no error during upload
    if (isset($cv_file) && $cv_file['error'] === 0) {
       ...
       // $file_name = time() . '-' . basename($cv_file['name']);
        ...
        if (move_uploaded_file($cv_file['tmp_name'], $new_file_path)) {
            ...
            //echo esc_url($upload_dir['baseurl'] . '/applications/' . $file_name);die();
